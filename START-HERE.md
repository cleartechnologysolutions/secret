...
4. Open the Worker → Settings → **Runtime variables and secrets** (not build variables). Add this as **Secret**:
   - `SMTP_PASSWORD`: your SMTP2GO SMTP user's password.
5. Save/deploy the settings. Open the Worker URL, create a test link addressed to yourself. Request a code, verify, reveal, and then confirm the same link cannot reveal again.
...
## Using it
Sender enters the intended email, a password/note, and expiration (5 minutes–48 hours). Copy the entire generated link, including the part after `#`, and send it to that recipient. Creating a link does not automatically email it.
...
- Anyone can create a link. Same-origin POSTs, no-store responses, CSP, and no external scripts.
- Per-IP rate limits: 30 API requests/minute, 5 new links/minute and 5 code requests/minute per recipient email. These are Cloudflare edge limits, not a global spending cap. Usage/provider charges follow your plans.
- Max note 8 KB UTF-8 (UI also limits to 4,000 characters). Standard ASCII email addresses supported.

## Troubleshooting
- Setup message: add secrets under Runtime variables, not Builds.
...
Tests cover encryption roundtrip, expiry, code limits/reuse, SMTP failure state, simultaneous reveal, public creation, origin validation, cache headers and GET safety. SMTP transport is tested with a simulated server. Runtime and browser checks were performed separately. Real SMTP2GO delivery must be verified after deployment.
...
## Updating from the login version
Replace the project files, including wrangler.json. ADMIN_PASSWORD is no longer used and can be removed from runtime secrets. Existing links remain usable, but previously issued codes/grants require a new verification code after this update. SMTP_PASSWORD is still required.
