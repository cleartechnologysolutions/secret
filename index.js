...
import { mac,equal,random,email,json } from './crypto.js';
...
async function handler(req,env){
 ...
 if(req.method==='GET'){
  ...
  if(p==='/api/session')return json({ready:ready(env),authenticated:await auth(req,env)});
 ...
 if(!ready(env))return json({error:'Setup needed: add ADMIN_PASSWORD and SMTP_PASSWORD in Worker runtime secrets.'},503);
 ...
 if(p==='/api/login'){
  if(env.LOGIN_LIMITER&&!(await env.LOGIN_LIMITER.limit({key:ip})).success)return json({error:'Too many sign-in attempts. Wait a minute.'},429);
  if(!equal(await mac(env.ADMIN_PASSWORD,String(b.password)),await mac(env.ADMIN_PASSWORD,env.ADMIN_PASSWORD)))return json({error:'Incorrect admin password.'},403);
  const expiry=String(Date.now()+28800000),token=expiry+'.'+await mac(env.ADMIN_PASSWORD,'session:'+expiry);
  return new Response('{"ok":true}',{headers:{'Content-Type':'application/json','Set-Cookie':'session='+token+'; HttpOnly; Secure; SameSite=Strict; Path=/; Max-Age=28800'}});
 }
 if(p==='/api/logout')return new Response('{}',{headers:{'Set-Cookie':'session=; HttpOnly; Secure; SameSite=Strict; Path=/; Max-Age=0'}});
 if(p==='/api/create'){
  if(!await auth(req,env))return json({error:'Sign in before creating a link.'},403);
 ...
 if(match){if(match[2]==='code'){if(!email(b.email))return json({error:'Enter a valid email.'},400);b.email=b.email.toLowerCase();}if(match[2]==='verify'&&!/^\d{6}$/.test(b.code))return json({error:'Enter the six-digit code.'},400);if(match[2]==='reveal'&&!/^[\w-]{43}$/.test(b.grant))return json({error:'Verification required.'},403);
