...
let lines=[],sent=[],connectionError=null;const encoder=new TextEncoder();let control;
globalThis.__smtpConnect=()=>{if(connectionError)throw connectionError;const readable=new ReadableStream({start(c){control=c;c.enqueue(encoder.encode('220 mock ready\r\n'));}});return{opened:new Promise(()=>{}),closed:Promise.resolve(),close:async()=>{},readable,writable:new WritableStream({write(bytes){const s=new TextDecoder().decode(bytes);sent.push(s);let response;if(s.startsWith('EHLO'))response='250-mock\r\n250 AUTH LOGIN\r\n';else if(s.startsWith('AUTH'))response='334 user\r\n';else if(sent.length===3)response='334 password\r\n';else if(sent.length===4)response='235 authenticated\r\n';else if(s==='DATA\r\n')response='354 continue\r\n';else response='250 accepted\r\n';control.enqueue(encoder.encode(response));}})};};
...
console.log('PASS SMTP with unresolved opened promise, multiline replies, authentication sequence, DATA termination and header injection rejection.');

connectionError=Error('TLS certificate validation failed for test-only');
await assert.rejects(()=>sendCode({SMTP_HOST:'mock',SMTP_PORT:'443',SMTP_FROM:'sender@example.com',SMTP_USER:'user',SMTP_PASSWORD:'test-only'},'client@example.com','123456'),e=>e.publicSmtpError&&e.message.includes('TLS certificate validation failed')&&!e.message.includes('test-only'));
console.log('PASS connection failure diagnostics and credential redaction.');
